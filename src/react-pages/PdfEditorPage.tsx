...
const iconBtnSx = {
  ...
  '&:focus-visible': { outline: '3px solid', outlineColor: 'primary.300', outlineOffset: 2 },
} as const;
...
export default function PdfEditorPage() {
  ...
  useEffect(() => {
    if (redactModalPage === null) return;
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setRedactModalPage(null);
    };
  ...
  return (
    <Box sx={{ maxWidth: 740, mx: 'auto', width: '100%', py: { xs: 3, md: 4 } }}>
                ...
                {pages.map((page, index) => pageMode === 'organize' ? (
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.25, width: '100%' }}>
                      ...
                      <Box component="button" aria-label={t`Delete page ${page.pageNum}`} onClick={(e: React.MouseEvent) => { e.stopPropagation(); deletePage(index); }}
                    ...
                    <Box sx={{ display: 'flex', gap: 0.25, alignItems: 'center' }}>
                      <Box component="button" aria-label={t`Rotate page ${page.pageNum} left`} onClick={(e: React.MouseEvent) => { e.stopPropagation(); rotatePage(index, -90); }} sx={iconBtnSx}><RotateLeftRoundedIcon sx={{ fontSize: 16 }} /></Box>
                      <Box component="button" aria-label={t`Rotate page ${page.pageNum} right`} onClick={(e: React.MouseEvent) => { e.stopPropagation(); rotatePage(index, 90); }} sx={iconBtnSx}><RotateRightRoundedIcon sx={{ fontSize: 16 }} /></Box>
                      <Dropdown>
                        <MenuButton slots={{ root: Box }} slotProps={{ root: { component: 'button', sx: iconBtnSx, title: t`Download page`, 'aria-label': t`Download page ${page.pageNum}`, onClick: (e: React.MouseEvent) => e.stopPropagation() } }}>
      ...
      {redactModalPage !== null && selectedFile && (
        <Box role="dialog" aria-modal="true" aria-label={t`Redaction editor`} tabIndex={-1} sx={{
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.5, bgcolor: 'background.surface', borderBottom: '1px solid', borderColor: 'divider' }}>
            <Box component="button" aria-label={t`Previous page`} onClick={() => setRedactModalPage(p => p !== null && p > 0 ? p - 1 : p)}
            ...
            <Box component="button" aria-label={t`Next page`} onClick={() => setRedactModalPage(p => p !== null && p < pages.length - 1 ? p + 1 : p)}
            ...
            <Box component="button" aria-label={t`Close redaction editor`} onClick={() => setRedactModalPage(null)} sx={iconBtnSx}>
              <CloseRoundedIcon />
            </Box>
          </Box>
