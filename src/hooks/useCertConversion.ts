...
const initialState: CertConversionState = {
  status: 'idle',
  jobId: null,
  position: 0,
  error: null,
...
export function useCertConversion(endpoint: string) {
  const [state, setState] = useState<CertConversionState>(initialState);
