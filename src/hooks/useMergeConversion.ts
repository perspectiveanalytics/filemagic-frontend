...
interface MergeConversionState {
...
const initialState: MergeConversionState = {
  status: 'idle',
  jobId: null,
  position: 0,
  error: null,
...
export function useMergeConversion(endpoint: string) {
  const [state, setState] = useState<MergeConversionState>(initialState);
